...
def train_unconditional_model(model, train_loader, fid_loader, config, output_dir):
    ...
    print(f"\n{'='*60}")
    print(f"Training Unconditional Model (All Data)")
    print(f"{'='*60}")
    ...
    for epoch in range(config.epochs):
        ...
        if (epoch + 1) % config.preview_real_every_epochs == 0:
            save_real_batch_preview(train_loader, f"{output_dir}/plots/real_preview_epoch{epoch+1}.png")
        ...
        save_dict = {
            ...
            'ema_model': model.ema_model.state_dict() if config.use_ema else None,
        ...
        # Snapshot theta_ref EMA
        if config.use_ema:
            torch.save(model.ema_model.state_dict(), f"{output_dir}/theta_ref_ema.pt")
            
        if is_best:
            torch.save(save_dict, f"{output_dir}/checkpoints/best.pt")
            if config.use_ema:
                torch.save(model.ema_model.state_dict(), f"{output_dir}/theta_ref_ema_best.pt")
...
def main():
    ...
    print("\n" + "="*60)
    print("UNCONDITIONAL DDPM PRETRAINING (ALL IMAGES)")
    print("="*60)
    ...
    batch = next(iter(preview_loader)).to(config.device)
    ...
    # Run training
    train_unconditional_model(model, train_loader, fid_loader, config, config.output_dir)
