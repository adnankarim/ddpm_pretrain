...
def generate_video(model, vae, noise_scheduler, control, fingerprint, prompt, save_path, device, num_frames=40):
    ...
    with torch.no_grad():
        # 1. Prepare Inputs
        dtype = torch.float16 if model.config.mixed_precision == "fp16" else torch.float32
        
        ctrl_latents = vae.encode(control.unsqueeze(0).to(device, dtype=dtype)).latent_dist.mode() * vae.config.scaling_factor
